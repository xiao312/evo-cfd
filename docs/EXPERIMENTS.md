...
## The two boundary-failure corrections are closed

The reviewer identified two defects in the work pushed just before the
consolidation milestone. Both are now fixed, tested and verified on the compute
host.

### The receipt heredoc expanded what it recorded

The receipt is written in two halves. The static half carries the job identity
and the exact argument vector; the dynamic half carries the timestamps, the
elapsed time and the exit code, which must be evaluated. The delimiter on the
static half had been changed to a quoted form, but the dynamic fields were still
*inside* it, so a quoted delimiter wrote them literally and the receipt reported
`started_at="$START_ISO"` — a receipt that described no run at all. The `printf`
lines that were meant to write them had also been split across two lines by an
escaped-newline collapse, so the shell saw a truncated `printf %s` and a stray
quoted string that wrote nothing.

The repair keeps the two halves apart by construction: the static half goes
through a quoted delimiter and contains only identity, budget and the argv; the
dynamic half is written by four `echo` appends. `echo` is used in preference to
`printf` specifically because it needs no backslash escape, which is what the
editor kept collapsing.

This is the defect the earlier regression test missed, and why: the test decoded
the command line out of the generated text without ever executing the script.
An unquoted delimiter and a quoted one both satisfy "a heredoc appears in the
output". The replacement test *runs* the generated script against a solver that
echoes each argument it received, and requires the observer's vector, the
receipt's argv and the joined command field to agree. The arguments deliberately
include command substitution, backticks, a variable reference, spaces, an empty
argument, an embedded quote and non-ASCII, so expansion or re-quoting shows up
as a difference.

A stale receipt generated by the pre-fix code was captured on the compute host
and showed `$(id -u)`, `` `pwd` `` and `$HOME` already expanded at write time —
the receipt describing a command that never ran. The fixed code records all
three literally while still writing evaluated timestamps below them.

One environment artefact is recorded rather than fixed: on Windows the observer
chain passes through the MSYS2 runtime, which can convert arguments between two
git-bash processes and was observed dropping a quote character from the
*observation*. The receipt and the executed command line remain byte-identical
there, so the identity the test asserts holds; only the observation layer is
affected. The test runs clean on the compute host.

### The digest measured a conventional guess, not the prepared inputs

The MASCOTTE preparer records each input as `{ source, sha256 }` where `source`
is target-relative (`cases/agile-80mm/0/T`). The digest's reader looked for a
`path` field the record never carried, found none, and silently fell back to six
conventional OpenFOAM dictionaries that do not include `0/T`. A changed `0/T`
therefore did not move the digest at all, and a request could be frozen against
a case identity that did not reflect the files the attempt depended on. The
reviewer demonstrated exactly this by changing `0/T` and observing no movement.

The record now carries case-relative `path` as the identity, with target-relative
`source` beside it as provenance only. The reader resolves `path` inside the
attempt directory, converts legacy records that carry `source` alone by
stripping the `cases/<variant>/` prefix, and records where the input list itself
came from. Three distinctions that were previously invisible are now part of the
hash: a declared list cannot coincide with an assumed conventional one, a
missing input (ENOENT) is distinct from an unreadable one, and a record that
declares no inputs is marked incomplete coverage rather than read as complete.

The preparer's manifest verification still uses `source`, which is correct: that
is the target-relative notion the manifest is written in. The two notions of
path are now explicit rather than conflated.

### Verification

At `7979d46` on the compute host: controller 259/259, egress 29/29,
rsih-adapter 38/38. Zero failures. The two files that fail locally (`episode`
and `smoke`) require the `@evocfd/rsih-adapter` package and are server-only by
design.

### The compute host went unreachable mid-session

While verifying the receipt fix, the server stopped answering on both SSH ports.
It still answered ICMP, so the host was up and the ports were dropped — a
host-side outage, not the egress tunnel, which carries no SSH. It recovered on
