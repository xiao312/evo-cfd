...
async function main(): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      "set -o pipefail",
      // The OpenFOAM environment is needed for Allcheck as much as for the
      // solver: checkMesh is a host-built binary the profile puts on PATH.
      // Errexit is off while it is sourced, because config.sh/aliases ends in
      // an unalias that fails non-interactively and would abort halfway.
      "set +e",
      `source ${ENV_FILE}`,
      "set -eu",
