...
async function main(): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      // The OpenFOAM bashrc references shell variables it does not itself define
      // (ZSH_NAME among them), so nounset is enabled only after the environment
      // is sourced. Enabling it earlier makes the env file itself fail.
      "set -eo pipefail",
      `source ${ENV_FILE}`,
      "set -u",
